...
public class CollectionController(DbContextGame dbContext) : ControllerBaseApi
{
    ...
    public async Task<IActionResult> GetAllCollectionAsync(CancellationToken cancellationToken)
    {
        ...
        List<DtoHero> heroes = await dbContext.Heroes.AsNoTracking().Where(a => a.UserId == userId).Select(a => new DtoHero(a.Id, a.UserId, a.BaseHeroId, a.GroupName, a.Level, a.ExperienceNow, a.Strength, a.Agility, a.Intelligence, a.CritChance, a.CritPower, a.Haste, a.Versality, a.EndurancePhysical, a.EnduranceMagical, a.Health1000, a.Damage, a.ResistDamagePhysical, a.ResistDamageMagical)).ToListAsync(cancellationToken: cancellationToken).ConfigureAwait(false);
