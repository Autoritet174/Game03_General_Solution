...
namespace Client
{
    internal static class HttpRequester
    {
        ...
        internal static void Init(string Application_dataPath)
        {
            _httpClient = new();

            Ini ini = Ini.Create(Path.Combine(Application_dataPath, @"GameData\Config\Main.ini"));
        ...
        public static async Task<JObject> GetResponceAsync(Uri uri, string jsonBody = "{}")
        {
            ...
            //if (!string.IsNullOrWhiteSpace(MemoryCache.JwtToken))//ИСПРАВИТЬ
            //{
            //    request.Headers.Authorization = new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", MemoryCache.JwtToken);//ИСПРАВИТЬ
            //}
            ...
            else
            {
                ...
                //var keyError = LocalizationManager.GetKeyError(resultJObject, argDict);//ИСПРАВИТЬ
                //await GameMessage.ShowLocaleAndWaitCloseAsync(keyError, argDict);//ИСПРАВИТЬ
            }
            //}
